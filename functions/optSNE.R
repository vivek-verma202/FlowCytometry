#' opt-SNE
#' modified from:
#'https://rdrr.io/github/milescsmith/dim.reduction.wrappers/src/R/optSNE.R
...
#' @param n_components integer Dimensions of the embedded space. Default: 2
...
optSNE <- function(rdf,
  ...
  optsne_module <- import(module = "MulticoreTSNE", delay_load = TRUE)
  if (is.null(n_jobs)){
    n_jobs <- detectCores()
  }
  optsne <- optsne_module$MulticoreTSNE(n_components = as.integer(n_components),

...
vizAtlas_Freq_Directional(CellMat = CM_nk,Build = Build_nk,
                          FreqMat = Build_nk[[2]],
                          Y = meta$FM[meta$panel=="nk"],
                          ToUse = ToUse_nk,
                          SampsToUse=NULL,numCore = 90,
                          layout = umap_nk,
                          outDir = "./plots/nk",numIter = 100); gc()
vizAtlas_Freq_Directional(CellMat = CM_bmd,Build = Build_bmd,
                          FreqMat = Build_bmd[[2]],
                          Y = meta$FM[meta$panel=="bmd"],
                          ToUse = ToUse_bmd,
                          SampsToUse=NULL,numCore = 90,
                          layout = umap_bmd,
                          outDir = "./plots/bmd",numIter = 100); gc()
vizAtlas_Freq_Directional(CellMat = CM_cyto,Build = Build_cyto,
                          FreqMat = Build_cyto[[2]],
                          Y = meta$FM[meta$panel=="cyto"],
                          ToUse = ToUse_cyto,
                          SampsToUse=NULL,numCore = 90,
                          layout = umap_cyto,
                          outDir = "./plots/cyto",numIter = 100); gc()
vizAtlas_Freq_Directional(CellMat = CM_chemo,Build = Build_chemo,
                          FreqMat = Build_chemo[[2]],
                          Y = meta$FM[meta$panel=="chemo"],
                          ToUse = ToUse_chemo,
                          SampsToUse=NULL,numCore = 90,
                          layout = umap_chemo,
                          outDir = "./plots/chemo",numIter = 100); gc()
vizAtlas_Freq_Directional(CellMat = CM_treg,Build = Build_treg,
                          FreqMat = Build_treg[[2]],
                          Y = meta$FM[meta$panel=="treg"],
                          ToUse = ToUse_treg,
                          SampsToUse=NULL,numCore = 90,
                          layout = umap_treg,
                          outDir = "./plots/treg",numIter = 100); gc()


# step 5: Make a functional pval maps
## nk
vizAtlas_Func(CellMat = CM_nk,Build = Build_nk,
              FuncMat = Build_nk[[1]],Y = meta$FM[meta$panel=="nk"],
              ToUse = ToUse_nk,SampsToUse=NULL,numCore = 90,
              layout = umap_nk,outDir = "./plots/nk",
              FuncNames = c("TIGIT","CD16","CD226","CD107a",
                            "CD159c","CD159a","CD314","CD96"),
              numIter = 100); gc()
vizAtlas_Func_Directional(CellMat = CM_nk,Build = Build_nk,
                          FuncMat = Build_nk[[1]],Y = meta$FM[meta$panel=="nk"],
                          ToUse = ToUse_nk,SampsToUse=NULL,numCore = 90,
                          layout = umap_nk,outDir = "./plots/nk",
                          FuncNames = c("TIGIT","CD16","CD226","CD107a","CD96"),
                          numIter = 100); gc()
## bmd
vizAtlas_Func(CellMat = CM_bmd,Build = Build_bmd,
              FuncMat = Build_bmd[[1]],Y = meta$FM[meta$panel=="bmd"],
              ToUse = ToUse_bmd,SampsToUse=NULL,numCore = 90,
              layout = umap_bmd,outDir = "./plots/bmd",
              FuncNames = c("CD14","CD11c","CD21","CD38","IgD",
                            "CD27","CD16","CD10","HLADR"),
              numIter = 100); gc()
vizAtlas_Func_Directional(CellMat = CM_bmd,Build = Build_bmd,
                          FuncMat = Build_bmd[[1]],Y = meta$FM[meta$panel=="bmd"],
                          ToUse = ToUse_bmd,SampsToUse=NULL,numCore = 90,
                          layout = umap_bmd,outDir = "./plots/bmd",
                          FuncNames = c("CD21","CD38","IgD",
                                        "CD27","CD16","HLADR"),
                          numIter = 100); gc()
## cyto
vizAtlas_Func(CellMat = CM_cyto,Build = Build_cyto,
              FuncMat = Build_cyto[[1]],Y = meta$FM[meta$panel=="cyto"],
              ToUse = ToUse_cyto,SampsToUse=NULL,numCore = 90,
              layout = umap_cyto,outDir = "./plots/cyto",
              FuncNames = c("IL.4","CD45RA","Ki.67","IFN.g","IL.2",
                            "IL.17A","Foxp3"),
              numIter = 100); gc()
vizAtlas_Func_Directional(CellMat = CM_cyto,Build = Build_cyto,
                          FuncMat = Build_cyto[[1]],Y = meta$FM[meta$panel=="cyto"],
                          ToUse = ToUse_cyto,SampsToUse=NULL,numCore = 90,
                          layout = umap_cyto,outDir = "./plots/cyto",
                          FuncNames = c("IL.4","CD45RA","IFN.g","IL.2",
                                        "IL.17A","Foxp3"),
                          numIter = 100); gc()
## chemo
vizAtlas_Func(CellMat = CM_chemo,Build = Build_chemo,
              FuncMat = Build_chemo[[1]],Y = meta$FM[meta$panel=="chemo"],
              ToUse = ToUse_chemo,SampsToUse=NULL,numCore = 90,
              layout = umap_chemo,outDir = "./plots/chemo",
              FuncNames = c("CD45RA","Ki.67","CXCR5","CCR6",
                            "CXCR3","CRTH2"),
              numIter = 100); gc()
vizAtlas_Func_Directional(CellMat = CM_chemo,Build = Build_chemo,
                          FuncMat = Build_chemo[[1]],Y = meta$FM[meta$panel=="chemo"],
                          ToUse = ToUse_chemo,SampsToUse=NULL,numCore = 90,
                          layout = umap_chemo,outDir = "./plots/chemo",
                          FuncNames = c("CXCR5","CCR6","CD45RA",
                                        "CXCR3","CRTH2"),
                          numIter = 100); gc()
## treg
vizAtlas_Func(CellMat = CM_treg,Build = Build_treg,
              FuncMat = Build_treg[[1]],Y = meta$FM[meta$panel=="treg"],
              ToUse = ToUse_treg,SampsToUse=NULL,numCore = 90,
              layout = umap_treg,outDir = "./plots/treg",
              FuncNames = c("CD25","FcRL3","TIGIT","Helios",
                            "Foxp3","CD127"),
              numIter = 100); gc()
vizAtlas_Func_Directional(CellMat = CM_treg,Build = Build_treg,
                          FuncMat = Build_treg[[1]],Y = meta$FM[meta$panel=="treg"],
                          ToUse = ToUse_treg,SampsToUse=NULL,numCore = 90,
                          layout = umap_treg,outDir = "./plots/treg",
                          FuncNames = c("CD25","FcRL3","TIGIT","Helios",
